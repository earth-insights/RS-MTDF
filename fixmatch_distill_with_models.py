...
def main():
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        total_distillation_loss = AverageMeter()
        ...
        for i, ((img_x, mask_x),
            ...
            pred, dis_loss = model(torch.cat((img_x, img_u_s)), compute_loss=True)
            pred_x, pred_u_s = pred.split([num_lb, num_ulb])
            ...
            loss = (loss_x + loss_u_s + dis_loss) / 3.0
            ...
            total_distillation_loss.update(dis_loss.item())
            ...
            if rank == 0:
                ...
                writer.add_scalar('train/dis_loss',dis_loss.item(), iters)
                writer.add_scalar('train/mask_ratio', mask_ratio, iters)

            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                logger.info('Iters: {:}, LR: {:.7f}, Total loss: {:.3f}, Loss x: {:.3f}, Loss s: {:.3f}, dis_loss:{:.3f}, Mask ratio: '
                            '{:.3f}'.format(i, optimizer.param_groups[0]['lr'], total_loss.avg, total_loss_x.avg, 
                                            total_loss_s.avg, total_distillation_loss.avg, total_mask_ratio.avg))
...
if __name__ == '__main__':
    main()
    #CUDA_VISIBLE_DEVICES=7 sh scripts/train.sh 1 856