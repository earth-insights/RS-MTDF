...
from model.semseg.dpt_distill_with_CLIP_SAM import DPT
import numpy as np
...
import os
config_path = "./configs/Potsdam.yaml"
...
def evaluate(model, loader, mode, cfg, multiplier=None, model_path=None):
    ...
    if model_path is not None:
        ...
        folder_name = parts[-2] if len(parts) >= 2 else "result"
    ...
    # === 只保留有效类，排除 class 6 ===
    valid_classes = [i for i in range(cfg['nclass']) if i != 0]
    iou_class_valid = iou_class[valid_classes]
    f1_class_valid = f1_class[valid_classes]
    pred_sum_valid = pred_sum[valid_classes]
    target_sum_valid = target_sum[valid_classes]

    mIoU = np.mean(iou_class_valid) * 100
    mF1 = np.mean(f1_class_valid) * 100
    total_pixels = np.sum(target_sum_valid)
    p0 = np.sum(intersection[valid_classes]) / (total_pixels + 1e-10)
    pe = np.sum(pred_sum_valid * target_sum_valid) / ((total_pixels ** 2) + 1e-10)
    kappa = (p0 - pe) / (1 - pe + 1e-10)

    print(f"mIoU: {mIoU:.2f}%")
    print(f"mF1: {mF1:.2f}%")
    print(f"Kappa: {kappa:.4f}")
    print("Per-class IoU & F1 (excluding class 0):")
    for i in valid_classes:
        print(f"Class {i}: IoU = {iou_class[i] * 100:.2f}%, F1 = {f1_class[i] * 100:.2f}%")
    ...
    with open(save_txt, "w") as f:
        ...
        f.write("Per-class results (excluding class 6):\n")
        for i in valid_classes:
            f.write(f"Class {i}: IoU = {iou_class[i] * 100:.2f}%, F1 = {f1_class[i] * 100:.2f}%\n")
